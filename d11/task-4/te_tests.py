...
- Handle API failures with logging and retry mechanisms
"""
...
def api_request(method: str, endpoint: str, json_data=None, params=None):
    ...
    try:
        ...
        if response.status_code == 429:
            retry_after = response.headers.get('X-Organization-Rate-Limit-Reset', '60')
...
def find_existing_test_id(test_name: str) -> Optional[int]:
    ...
    try:
        ...
        logger.error(f"Failed to retrieve tests: {e}")
...
def analyze_results(results: Dict[str, Any]) -> None:
    ...
    This function demonstrates how to extract and display key metrics
    from ThousandEyes test results in a readable format.
...
def main():
    """
    Main execution function that orchestrates the ThousandEyes test workflow.
    
    This function coordinates all test operations including agent discovery,
    test creation, result retrieval, and report generation.
    """
    ...
    # Check for existing test or create new one
    ...
    if test_id is None:
        ...
        logger.info(f"Using existing test '{TEST_NAME}' (ID: {test_id})")
    
    # Wait for results if new test
    if is_new:
        wait_time = 90
        logger.info(
            f"Waiting {wait_time} seconds for the first test result to become available..."
        )
        time.sleep(wait_time)
    
    # Fetch and analyze results with retry
    max_attempts = 3
    for attempt in range(max_attempts):
        ...
        logger.error(f"No results returned after {max_attempts} attempts. Exiting.")
        sys.exit(1)
